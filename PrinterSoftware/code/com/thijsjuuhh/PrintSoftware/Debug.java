...
public class Debug implements Runnable {
	...
	private void render() {
		...
		int x = 20;
		int y = window.getHeight() - typeBarY;
		int color = 0;
		...
		for(int i = commands.size() - 1; i >= 0; i--) {
			if(commands.get(i).charAt(0) == Character.forDigit(Logger.INFO, 10))
				...
				color = 0xFF00FF00;
			graphics.render2d.renderText(commands.get(i).substring(1), x, y, Textures.basic, (window.getHeight() - typeBarY - 20)/ maxMessages, color);
